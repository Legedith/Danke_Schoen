import random
import markovify  # Use Markov models to generate new sentences
from rhyme import rhyme_finder  # Use cmudict to find phonic rhymes
import tqdm
import os
...
class Rapper:
    ...
    def load_data(self, data_dir=None):
        """Produce lyrics from loaded data.
        ...
        lyrics_files_paths = [f'{data_dir}/{file_path}'
    ...
    def build_space(self, lyrics_data, sentences_number=5000, state_size=2):
        """Produce sentences from loaded lyrics.
    ...
    def build_rhyme_dict(self, sentence):
        try:
            ...
            rhymes_dict = {'line': sentence,
                            'last_word' : last_word,
                            'rhymes': [last_word]
                            + rhyme_finder(last_word,self.tokenized_text)
                            }
            return rhymes_dict
        except Exception:
            pass
    ...
    def build_rhyme_list(self, sentences, show_errors=False):
        """Produce rhymes from loaded sentences.
    ...
    def build_equivalence_classes(self, rhymes_list):
        """Produce rhyme equivalence classes from loaded rhymes and lyrics.
        ...
        Output: List of dict of equivalent classes grouped by rhymes
                    [{'be': [{'line': 'All the pain I thought we could be',
                              'last_word': 'be',
                              'rhymes': ['be', 'me', 'somebody', ...]}, ...],
                      'me': [{'line': 'Do you feel me?',
                              'last_word': 'me',
                              'rhymes': ['me','baby','somebody','only',...]},
                        ...]
        """
        equivalence_list = []
        k = 0
        while len(rhymes_list) > 0:
            ...
            # Loop over all sentences to group by rhymes and last word
    ...
    def train(self, lyrics_data,
              sentences_number=5000, state_size=2,
              mod='artistic'):
        """Train model of equivalence classes from lyrics"""
        print(f'Generating a space of {sentences_number} sentences')
        sentences = self.build_space(lyrics_data,
                                     sentences_number, state_size)
    ...
    def generate_random_pair(self, cluster):
        ...
        rhyme1_cluster = random.choice(values_cluster[0])
        rhyme2_cluster = random.choice(values_cluster[1])
        return rhyme1_cluster['line'], rhyme2_cluster['line']

    def generate_artistic_verses(self, N_VERSES=8):
        patterns = ['abab', 'aabb', 'abba']
        generated_lyrics = ''
        equivalence_clusters = [cluster for cluster
                                in self.equivalence_list
                                if len(cluster.keys()) > 1]
        for _ in range(N_VERSES):
            pattern = random.choice(patterns)
            random.shuffle(equivalence_clusters)
            cluster1 = equivalence_clusters[0]
            cluster2 = equivalence_clusters[1]
    ...
    def generate_freestyle_verses(self, N_VERSES=8):
        verse_length = 4
        index = random.choice(range(len(self.free_style_sentences)
                                    -N_VERSES*verse_length-1))
        verses = self.free_style_sentences[index:index+N_VERSES*verse_length]
        verses = [verses[i:i+verse_length]
                  for i in range(0, len(verses), verse_length)]
        generated_lyrics = ''
